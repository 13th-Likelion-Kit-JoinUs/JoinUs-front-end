export const PositionComment = [
	'아기 사자들, 나만 따라와~',
	'우리 논리적으로 생각해볼까?',
	'순발력과 대응력은 나에게 맡겨!',
	'내가 예쁘게 만들어 줄게~',
];

export const PositionImage = [
	'/~s20231073/assets/img/PM_lion.png',
	'/~s20231073/assets/img/backend_lion.png',
	'/~s20231073/assets/img/frontend_lion.png',
	'/~s20231073/assets/img/desinger_lion.png',
];

export const PositionWord = [
	'/~s20231073/assets/img/PM_word.png',
	'/~s20231073/assets/img/backend_word.png',
	'/~s20231073/assets/img/frontend_word.png',
	'/~s20231073/assets/img/design_word.png',
];

export const Word = [
	'/~s20231073/assets/img/기획력.png',
	'/~s20231073/assets/img/분석력.png',
	'/~s20231073/assets/img/실행력.png',
	'/~s20231073/assets/img/창의력.png',
];

export const PositionWish = [
	'PM이 되려면...',
	'BACKEND 개발자가 되려면...',
	'FRONTEND 개발자가 되려면...',
	'UIUX DESIGNER가 되려면...',
];

export const Position = ['기획', '백엔드', '프론트엔드', '디자인'];
